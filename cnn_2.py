...
class Net(nn.Module):
    def __init__(self):
        ...
        self.backbone = nn.Sequential(
          ...
          nn.Conv2d( 3,5, 3, padding=1,stride=1), nn.ReLU(),
          nn.MaxPool2d(kernel_size=2, stride=2),
          nn.Conv2d(5,5, 3, padding=1,stride=1), nn.ReLU(),
          nn.MaxPool2d(kernel_size=2, stride=2),
          nn.Conv2d(5,7, 3, padding=1,stride=1), nn.ReLU(),
          nn.MaxPool2d(kernel_size=2, stride=2),
          nn.Conv2d(7,7, 3, padding=1,stride=1), nn.ReLU(),
          nn.MaxPool2d(kernel_size=2, stride=2),
          nn.Conv2d(7,9, 3, padding=1,stride=1), nn.ReLU(),
          nn.MaxPool2d(kernel_size=2, stride=2),
          nn.Conv2d(9,9, 3, padding=1,stride=1), nn.ReLU(),
          nn.MaxPool2d(kernel_size=2, stride=2),
          nn.Flatten()
          )
...
dst = labelFpsDataLoader([carpeta_prova_train], imgSize, transform)
...
torch.save(model.state_dict(), PATH + "model7dt2trans.pth")


#%% CARREGAR WEIGHTS MODEL
PATH = "C:/Users/xavid/Documents/uni/DADES/2n any/2n sem/PSIV/projecte/"
model = Net()
model.load_state_dict(torch.load(PATH + "modelsmodel7dt2trans.pth"))
