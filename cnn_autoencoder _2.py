...
torch.save(model.state_dict(), PATH + "model2.pth")
...
model = Net()
model.load_state_dict(torch.load(PATH + "modelsmodel2.pth"))

X = np.zeros([len(train_loader.dataset),512])
...
model = Net()
model.load_state_dict(torch.load(PATH + "modelsmodel2.pth"))
svm_model = pickle.load(open(PATH + "finalized_model.sav", 'rb'))


#%% TESTING TEST SET

X = np.zeros([len(test_loader.dataset),512])
...
#%% 
p1 = "C:/Users/xavid/Documents/uni/DADES/2n any/2n sem/PSIV/projecte/multi_dataset/Dataset/test/airplanes/airplane2.jpg"
...
def mira(img):
    ...
    #result2 = cv2.resize(result2, [img_size[1]/2,img_size[0]/2])
